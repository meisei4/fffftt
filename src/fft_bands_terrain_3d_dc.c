...
static void build_bands_terrain(const float* bin_levels);
...
static void build_glitter_color_field(const float* bin_levels);
...
static void update_playback_controls_fft_bands(void);
...
int main(void) {
    ...
    // wave = LoadWave(RD_CT_LOR_22K_WAV);
    // wave = LoadWave(RD_AT_UNTITLED_22K_WAV);
    // wave = LoadWave(RD_TJ_SAYO_22K_WAV);
    ...
    update_mesh_colors_glitter(high_band_colors, &high_band_glitter_field[0][0], HIGH_BAND_POINT_COUNT, 0.0f);
    ...
    while (!WindowShouldClose()) {
        ...
        update_playback_controls_fft_bands();
        ...
        if (audio_dirty) {
            ...
            update_mesh_vertices(high_band_vertices, &high_band_lane_point_values[0][0], HIGH_BAND_POINT_COUNT);
            update_mesh_normals_smooth(high_band_normals, high_band_vertices, HIGH_BAND_POINT_COUNT);
            update_mesh_colors_glitter(high_band_colors, &high_band_glitter_field[0][0], HIGH_BAND_POINT_COUNT, (float)GetTime());
            build_mesh_smooth(&high_band_mesh, high_band_vertices, high_band_normals, high_band_colors, HIGH_BAND_VERTEX_COUNT);
        }
...
static void build_bands_terrain(const float* bin_levels) {
    build_band_terrain(&low_band_lane_point_values[0][0], LOW_BAND_POINT_COUNT, bin_levels, LOW_BAND_BIN_BOUNDS);
    build_band_terrain(&mid_band_lane_point_values[0][0], MID_BAND_POINT_COUNT, bin_levels, MID_BAND_BIN_BOUNDS);
    build_band_terrain(&high_band_lane_point_values[0][0], HIGH_BAND_POINT_COUNT, bin_levels, HIGH_BAND_BIN_BOUNDS);
    ...
    build_glitter_color_field(bin_levels);
}
...
static void build_glitter_color_field(const float* bin_levels) {
    ...
    const int lag_index = (fft_data.history_pos - 1 - ONSET_LAG_FRAMES + ANALYSIS_FFT_HISTORY_FRAME_COUNT) % ANALYSIS_FFT_HISTORY_FRAME_COUNT;
    const float* lag_spectrum = (fft_data.frame_index > ONSET_LAG_FRAMES) ? fft_data.spectrum_history_levels[lag_index] : bin_levels;
    ...
    for (int i = 0; i < HIGH_BAND_POINT_COUNT; i++) {
        ...
        const float prev = high_band_lane_point_values[0][prev_i];
        const float next = high_band_lane_point_values[0][next_i];
        const float ridge = high_band_lane_point_values[0][i];
    ...
    for (int i = 0; i < HIGH_BAND_POINT_COUNT; i++) {
        ...
        const float decay = high_band_glitter_field[1][i] * 0.84f;
...
static void update_mesh_colors_glitter(Color* colors, const float* glitter_field, int point_count, float t) {
    for (int i = 0; i < LANE_COUNT; i++) {
        for (int j = 0; j < point_count; j++) {
            ...
            const float phase = t * 10.5f + j * 0.85f + i * 1.15f + glitter * 11.0f;
            ...
            colors[i * point_count + j] =
                (Color){DRAW_COLOR_CHANNEL_MAX, DRAW_COLOR_CHANNEL_MAX, DRAW_COLOR_CHANNEL_MAX, (unsigned char)(alpha * (float)DRAW_COLOR_CHANNEL_MAX)};
        }
    }
}
...
static void consume_latest_fft_history_frame(void) {
    ...
    advance_lane_history(&high_band_glitter_field[0][0], HIGH_BAND_POINT_COUNT);
    build_bands_terrain(bin_levels);
...
static void update_playback_controls_fft_bands(void) {
    ...
    if (rebuild_from_cursor) {
        ...
        MEMSET(high_band_glitter_field, 0, sizeof(high_band_glitter_field));
