...
FFT_BAND_TERRAIN_3D_DC_TARGET_DIR := $(BUILD_DIR)/dc/fft_bands_terrain_3d_dc
...
FFT_BAND_TERRAIN_3D_DC_LAUNCHER := $(BIN_DIR)/fft_bands_terrain_3d_dc
...
FFT_BAND_TERRAIN_3D_DC_TARGET := $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/fft_bands_terrain_3d_dc.elf
...
FFT_BAND_TERRAIN_3D_DC_SOURCE := $(SRC_DIR)/fft_bands_terrain_3d_dc.c
...
.PHONY: help all cool-gl33 fftw-gl11 cool-gl11 cool-dc sh4zam-butterfly fftw-dc audio-only-wav-dc audio-only-mp3-dc picking-out-notes-dc waveform-dc sound-envelope-dc sound-envelope-3d-dc sound-envelope-gl33 sound-envelope-3d-audio-cadence-dc waveform-terrain-3d-dc fft-terrain-3d-dc fft-bands-terrain-3d-dc gldc-mesh-draw-mode-dc fmt clean-all
help:
	$(error PLEASE DEFINE A TARGET: make cool-gl33 | make cool-gl11 | make fftw-gl11 | make cool-dc | make sh4zam-butterfly | make fftw-dc | make audio-only-wav-dc | make audio-only-mp3-dc | make picking-out-notes-dc | make waveform-dc | make sound-envelope-dc | make sound-envelope-3d-dc | make sound-envelope-gl33 | make sound-envelope-3d-audio-cadence-dc | make waveform-terrain-3d-dc | make fft-terrain-3d-dc | make fft-bands-terrain-3d-dc | make fmt | make clean-all)
...
#	cp -f $(SRC_DIR)/resources/shadertoy_electronebulae_one_fourth_22050hz_pcm16_mono.wav $(WAVEFORM_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
...
#	cp -f $(SRC_DIR)/resources/vga_rom_f16_1px_tight.png $(WAVEFORM_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
	cp -f $(SRC_DIR)/resources/vga_rom_f16_0px_TIGHT.fnt $(WAVEFORM_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
	cp -f $(SRC_DIR)/resources/vga_rom_f16_0px_TIGHT.png $(WAVEFORM_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
...
#	cp -f $(SRC_DIR)/resources/shadertoy_electronebulae_one_fourth_22050hz_pcm16_mono.wav $(FFT_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
...
#	cp -f $(SRC_DIR)/resources/vga_rom_f16_1px_tight.png $(FFT_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
	cp -f $(SRC_DIR)/resources/vga_rom_f16_0px_TIGHT.fnt $(FFT_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
	cp -f $(SRC_DIR)/resources/vga_rom_f16_0px_TIGHT.png $(FFT_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
...
#	cp -f $(SRC_DIR)/resources/vga_rom_f16_1px_tight.png $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
	cp -f $(SRC_DIR)/resources/vga_rom_f16_0px_TIGHT.fnt $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
	cp -f $(SRC_DIR)/resources/vga_rom_f16_0px_TIGHT.png $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk/
	$(KOS_GENROMFS) -f $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk.img -d $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk -v -x .gitignore -x .DS_Store -x Thumbs.db
	$(KOS_BASE)/utils/bin2c/bin2c $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk.img $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk_tmp.c romdisk
	$(KOS_CC) $(KOS_CFLAGS) -o $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk_tmp.o -c $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk_tmp.c
	$(KOS_CC) -o $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk.o -r $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk_tmp.o \
	  -L$(KOS_BASE)/lib/$(KOS_ARCH) -L$(KOS_BASE)/addons/lib/$(KOS_ARCH) \
	  -L$(KOS_PORTS)/lib -Wl,--whole-archive -lromdiskbase
	kos-cc -iquote sh4zam/include -I$(DC_RAYLIB_DIR) -I$(SRC_DIR) -I$(KOS_PORTS)/libwav/inst/include -DPLATFORM_DREAMCAST -DGRAPHICS_API_OPENGL_11 $(filter-out $(KOS_PORTS_INCLUDE),$(KOS_CFLAGS)) $(GLDC_INCLUDE_CFLAG) $(FFT_FLAGS) -std=gnu2x -c $(FFT_BAND_TERRAIN_3D_DC_SOURCE) -o $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/fft_bands_terrain_3d_dc.o
	kos-cc -iquote sh4zam/include -DPLATFORM_DREAMCAST $(filter-out $(KOS_PORTS_INCLUDE),$(KOS_CFLAGS)) $(GLDC_INCLUDE_CFLAG) -std=gnu2x -c $(SH4ZAM_COMPLEX_SOURCE) -o $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/shz_complex.o
	kos-cc -o $(FFT_BAND_TERRAIN_3D_DC_TARGET) $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/fft_bands_terrain_3d_dc.o $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/shz_complex.o $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk.o $(DC_RAYLIB_DIR)/libraylib.a $(GLDC_LIB) -lkosutils -lwav -lm -lpthread
	$(call WRITE_BIN,$(FFT_BAND_TERRAIN_3D_DC_TARGET),$(FFT_BAND_TERRAIN_3D_DC_LAUNCHER))
	rm -f $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk_tmp.c $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk_tmp.o
	rm -rf $(FFT_BAND_TERRAIN_3D_DC_TARGET_DIR)/romdisk
...
clean-all:
	rm -rf $(BUILD_DIR)
	rm -f $(GL33_BIN) $(FFTW_BIN) $(GL11_BIN) $(DC_LAUNCHER) $(SH4_LAUNCHER) $(DC_FFTW_LAUNCHER) $(AUDIO_ONLY_WAV_LAUNCHER) $(AUDIO_ONLY_WAV_LAUNCHER)_1024 $(AUDIO_ONLY_WAV_LAUNCHER)_2048 $(AUDIO_ONLY_MP3_LAUNCHER) $(AUDIO_ONLY_MP3_LAUNCHER)_1024 $(AUDIO_ONLY_MP3_LAUNCHER)_2048 $(PICKING_OUT_NOTES_DC_LAUNCHER) $(WAVEFORM_DC_LAUNCHER) $(WAVEFORM_SANDBOX_DC_LAUNCHER) $(SOUND_ENVELOPE_DC_LAUNCHER) $(SOUND_ENVELOPE_3D_DC_LAUNCHER) $(SOUND_ENVELOPE_3D_AUDIO_CANDENCE_DC_LAUNCHER) $(SOUND_ENVELOPE_GL33_LAUNCHER) $(WAVEFORM_TERRAIN_3D_DC_LAUNCHER) $(FFT_TERRAIN_3D_DC_LAUNCHER) $(FFT_BAND_TERRAIN_3D_DC_LAUNCHER) $(DC_MESH_DRAW_MODE_LAUNCHER) $(DC_TARGET) $(SH4_TARGET) $(DC_FFTW_TARGET) $(AUDIO_ONLY_WAV_TARGET) $(AUDIO_ONLY_MP3_TARGET) $(PICKING_OUT_NOTES_DC_TARGET) $(WAVEFORM_DC_TARGET) $(WAVEFORM_SANDBOX_DC_TARGET) $(SOUND_ENVELOPE_DC_TARGET) $(SOUND_ENVELOPE_3D_DC_TARGET) $(SOUND_ENVELOPE_GL33_TARGET) $(SOUND_ENVELOPE_3D_AUDIO_CANDENCE_DC_TARGET) $(WAVEFORM_TERRAIN_3D_DC_TARGET) $(FFT_TERRAIN_3D_DC_TARGET) $(FFT_BAND_TERRAIN_3D_DC_TARGET) $(DC_MESH_DRAW_MODE_TARGET)
	rm -rf logs/*
