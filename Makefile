...
SH4ZAM_BUILD_DIR := $(DC_BUILD_DIR)/sh4zam
SH4ZAM_LIB := $(SH4ZAM_BUILD_DIR)/libsh4zam.a
...
$(DC_ELFS): TARGET_CFLAGS := $(DC_BASE_CFLAGS) $(DC_FFT_CFLAGS)
...
$(SH4ZAM_BUTTERFLY_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(SH4ZAM_BUTTERFLY_DC_ELF): $(SH4ZAM_BUTTERFLY_SRC) $(SH4ZAM_LIB)
...
$(PICKING_OUT_NOTES_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(PICKING_OUT_NOTES_DC_ELF): $(PICKING_OUT_NOTES_SRC) $(SH4ZAM_LIB)

$(WAVEFORM_DC_ELF): SRC := $(WAVEFORM_SRC)
$(WAVEFORM_DC_ELF): TARGET_CPPFLAGS := $(SH4ZAM_CPPFLAGS) $(DC_CPPFLAGS)
$(WAVEFORM_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(WAVEFORM_DC_ELF): $(WAVEFORM_SRC) $(SH4ZAM_LIB)

$(SOUND_ENVELOPE_2D_ISO_DC_ELF): SRC := $(SOUND_ENVELOPE_2D_ISO_SRC)
$(SOUND_ENVELOPE_2D_ISO_DC_ELF): TARGET_CPPFLAGS := $(SH4ZAM_CPPFLAGS) $(DC_CPPFLAGS)
$(SOUND_ENVELOPE_2D_ISO_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(SOUND_ENVELOPE_2D_ISO_DC_ELF): $(SOUND_ENVELOPE_2D_ISO_SRC) $(SH4ZAM_LIB)

$(SOUND_ENVELOPE_3D_DC_ELF): SRC := $(SOUND_ENVELOPE_3D_SRC)
$(SOUND_ENVELOPE_3D_DC_ELF): TARGET_CPPFLAGS := $(SH4ZAM_CPPFLAGS) $(DC_CPPFLAGS)
$(SOUND_ENVELOPE_3D_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(SOUND_ENVELOPE_3D_DC_ELF): $(SOUND_ENVELOPE_3D_SRC) $(SH4ZAM_LIB)

$(WAVEFORM_TERRAIN_3D_DC_ELF): SRC := $(WAVEFORM_TERRAIN_3D_SRC)
$(WAVEFORM_TERRAIN_3D_DC_ELF): TARGET_CPPFLAGS := $(SH4ZAM_CPPFLAGS) $(DC_CPPFLAGS)
$(WAVEFORM_TERRAIN_3D_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(WAVEFORM_TERRAIN_3D_DC_ELF): $(WAVEFORM_TERRAIN_3D_SRC) $(SH4ZAM_LIB)

$(FFT_TERRAIN_3D_DC_ELF): SRC := $(FFT_TERRAIN_3D_SRC)
$(FFT_TERRAIN_3D_DC_ELF): TARGET_CPPFLAGS := $(SH4ZAM_CPPFLAGS) $(DC_CPPFLAGS)
$(FFT_TERRAIN_3D_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(FFT_TERRAIN_3D_DC_ELF): $(FFT_TERRAIN_3D_SRC) $(SH4ZAM_LIB)
...
$(FFT_BANDS_TERRAIN_3D_DC_ELF): EXTRA_OBJS := $(SH4ZAM_LIB)
$(FFT_BANDS_TERRAIN_3D_DC_ELF): $(FFT_BANDS_TERRAIN_3D_SRC) $(SH4ZAM_LIB)
...
SH4ZAM_LOCAL_BUILD := env -u CPATH -u LIBRARY_PATH KOS_CFLAGS="-I$(abspath $(SH4ZAM_DIR))/include $(filter-out -I$(KOS_PORTS)/include,$(KOS_CFLAGS))" KOS_INC_PATHS="-I$(abspath $(SH4ZAM_DIR))/include $(filter-out -I$(KOS_PORTS)/include,$(KOS_INC_PATHS))"

$(SH4ZAM_LIB):
	@test -f $(SH4ZAM_DIR)/CMakeLists.txt || { echo "SH4ZAM checkout missing at $(SH4ZAM_DIR). Initialize the submodule first."; exit 1; }
	cd $(SH4ZAM_DIR) && $(SH4ZAM_LOCAL_BUILD) $(CMAKE) --fresh -G "$(CMAKE_GENERATOR)" -S . -B $(abspath $(SH4ZAM_BUILD_DIR)) -DCMAKE_TOOLCHAIN_FILE=$(GLDC_TOOLCHAIN) -DCMAKE_BUILD_TYPE=Release
	$(SH4ZAM_LOCAL_BUILD) $(CMAKE) --build $(SH4ZAM_BUILD_DIR) --parallel
	@test -f $(SH4ZAM_LIB)
